---
People:
tags:
  - acroneberger
  - ntalasila
  - bkartchner
  - abensasson
  - jamon
Follow up:
---


---
## 🗓️ Agenda 
*What are we talking about today?*



## ✍️ Notes and Action Items
*Things that we talked about*

- modal incorporation is done through docker container
	- [ ] follow up with #acroneberger on the modal enterprise agreement 📅 2026-01-14 

- pymol viz additions from Ariel
	- Shared Miro board (check this)


Optimization strategies
- start from de novo
- Fix motif (energetically favorable interactions — 3 aas) and let RF diffusion to build around it
- stretch of structural motifs —> not just individual amino acids that are disjointed